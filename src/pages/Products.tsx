...
export const Products: React.FC = () => {
    ...
    const handlePrintLabel = async (variant: any, product: any) => {
        const copies = parseInt(prompt('How many labels to print?', '1') || '0');
        if (!copies || copies <= 0) return;
        ...
        try {
            ...
            // Smart Stock: Ask to add to stock
            if (confirm(`Do you want to add ${copies} items to stock?`)) {
                await db.productVariants.update({
                    where: { id: variant.id },
                    data: { stock: { increment: copies } }
                });
                ...
                await db.inventoryMovements.create({
                    data: {
                        ...
                        type: 'IN',
                        quantity: copies,
                        reason: 'Label Print Auto-Stock',
                        createdBy: 'SYSTEM' // simpler than getting user context here
                    }
                });
                ...
                await auditService.log(
                    'STOCK_ADD',
                    `Added ${copies} stock for ${product.name} via Label Print`
                );
