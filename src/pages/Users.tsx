...
export const Users: React.FC = () => {
    ...
    const handleSaveUser = async () => {
        ...
        try {
            ...
            if (editingUser) {
                ...
                // Create
                res = await window.electronAPI.users.create(formData);
            }
    ...
    const handleSavePassword = async () => {
        ...
        try {
            const res = await window.electronAPI.users.changePassword(passwordData.userId, passwordData.newPassword);
            if (res.success) {
                setShowPasswordModal(false);
                setPasswordData({ userId: '', currentPassword: '', newPassword: '', confirmPassword: '' });
    ...
    const handleToggleStatus = async (user: any) => {
        if (confirm(`Are you sure you want to ${user.isActive ? 'deactivate' : 'activate'} this user?`)) {
            const res = await window.electronAPI.users.update(user.id, { isActive: !user.isActive });
    ...
    return (
        <div className="space-y-6">
            ...
            >
                <div className="space-y-4">
                    <PasswordInput
                        label="Current Password (Read Only)"
